...
class QAGNN(nn.Module):
    def __init__(
        ...
        self.fc = MLP(
            input_size=concept_dim + sent_dim + concept_dim,
            hidden_size=fc_dim,
            output_size=2,
    ...
    def forward(
        ...
        graph_vecs, _ = self.pooler(sent_vecs, gnn_output, mask)

        # when predicting relevance of a graph, we consider: 
        # pooled graph representation, LM embedding and embedding of QAGNN_contextnode
        # (batch_size, dim_node + dim_sent + dim_node)
        concat = torch.cat((graph_vecs, sent_vecs, sent_vecs_after_gnn), dim=-1)
...
class QAGNNGraphClassifier(nn.Module):
    def __init__(
        self,
        encoder_name: str,
        gnn_name: str,
